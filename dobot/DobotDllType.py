...
def load():
    if platform.system() == "Windows":
        ...
        return CDLL("libDobotDll.dylib",  RTLD_GLOBAL)
          ...
          # 方式一：把对应版本的Linux动态库的所有文件放在执行文件目录下
          # 终端执行：$ python3 DobotDllType.py
          # 终端执行：$ python3 DobotControl.py
        return CDLL("libDobotDll.so") 
        
        # 方式二：在~/.bashrc件把对应版本的Linux动态库的路径添加环境变量；
        # eg: export DYLD_LIBRARY_PATH=$DYLD_LIBRARY_PATH: DOBOT_LIB_PATH    注：DOBOT_LIB_PATH->动态库的绝对路径
            # $ source ~/.bashrc  注：source更新一下环境变量，零开终端即可生效！
            # 终端执行：$ python3 DobotDllType.py
          # 终端执行：$ python3 DobotControl.py
